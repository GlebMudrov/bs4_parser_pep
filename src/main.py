...
def pep(session):
    ...
    for tag in tqdm(tr_tags):
        pre_status = find_tag(tag, 'abbr').text[1:]
        full_status = EXPECTED_STATUS[pre_status]
        tags_a = find_tag(tag, 'a', attrs={'class': 'pep reference internal'})
        pep_link = urljoin(PEPS_URL, tags_a['href'])
        ...
        if status not in full_status:
            logs.append([pep_link, status, full_status])

    if logs:
        for log in logs:
            logging.info(
                    f'Несовпадающие статусы:\n{log[0]}\n'
                    f'Статус в карточке: {log[1]}\n'
                    f'Ожидаемые статусы: {", ".join(log[2])}'
            )
